...
namespace coen79_lab6
{
	...
	char string::operator[](size_t pos) const
	{
	 ...
	 void string::insert(const string& source, unsigned int position)
	 {
	 	...
	 	reserve(this->length() + source.length() + 1);
	 	size_t i = this->allocated - 1;
	 	while (i >= position + source.length())
	 	{
	 		this -> sequence[i] = this -> sequence [i - source.length()];
	 		--i;
	 	}
	 	strncpy(this->sequence + position, source.sequence, source.length());
	 }

	 void string::dlt(unsigned int position, unsigned int num)
	 {
	 	assert(position + num < length());
	 	size_t i = position + num;
	 	while(i < this->length())
	 	{
	 ...
	 void string::replace(char c, unsigned int position)
	 {
	  ...
	  {
	  	assert(position + source.length() <= this -> length());
	  	size_t i = 0;
	  	while(i < source.length())
	  	{
	  		sequence[position + i] = source.sequence[i];
	  		++i;
	  	}
	  }
	  ...
	  {
	  	int i = 0;
	  	while(i < this->length() - substring.length())
	  	{
	  		if(strncmp(substring.sequence,sequence + i, substring.length()) == 0)
	  		{
	  			return i;
	  		}
	  		++i;
